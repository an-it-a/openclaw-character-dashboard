...
export function InspectorPanel(): JSX.Element {
  ...
  if (!selection) {
    return (
      <aside className="inspector-panel inspector-panel--empty">
        <p className="inspector-panel__hint">Click a character or room on the map</p>
      </aside>
    );
  }
  ...
  if (selection.type === "character") {
    ...
    const charConfig = worldConfig?.characters.find((c) => c.id === selection.characterId);
    const room = worldConfig?.rooms.find((r) => r.id === charState?.currentRoomId);

    return (
      <aside className="inspector-panel">
        <h2 className="inspector-panel__title">{charConfig?.name ?? selection.characterId}</h2>
        <dl className="inspector-panel__details">
          ...
          <dt>Private room</dt>
          <dd>
            {worldConfig?.rooms.find((r) => r.id === charConfig?.privateRoomId)?.label ??
  ...
  if (selection.type === "room") {
    ...
    return (
      <aside className="inspector-panel">
        <h2 className="inspector-panel__title">{room?.label ?? selection.roomId}</h2>
        <dl className="inspector-panel__details">
          <dt>ID</dt>
          <dd>{selection.roomId}</dd>
          <dt>Size</dt>
          <dd>
            {room ? `${room.width} × ${room.height} px` : "—"}
          </dd>
          <dt>Objects</dt>
